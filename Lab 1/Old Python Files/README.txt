...
The Python files to be used in the Niryo One Simulater should be run with python <file_name>.py to allow for the output files to be saved.
...
- File test_kinematics.py: this Python script uses the Niryo One api and simulated the robot trajectory without any noise. It also uses the Direct and Inverse Kinematics that were developed. This script outputs two files: output.txt (which has the estimated positions and joints with the kinematics) and commands_test_kin.txt (which has, for each time, the time, command and arguments for an action that the robot makes);

- File error_calc.py: this Python script uses the file output.txt to calculate the errors between the position estimated with the Direct Kinematics and the actual positions and the errors between the joints estimated with the Inverse Kinematics and the actual joints;
