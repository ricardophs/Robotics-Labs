function [path_m_ref, final_path] = path_planning(draw, resolution)
    ...
    % Default path 3
%     start_default = [755, 458];
%     end_default = [687, 837];
    
    % Default path 2
%     start_default = [415, 516];
%     end_default = [750, 546];
    ...
    load('DataFiles/near_nodes_matrix.mat', 'near_nodes_matrix');
    load('DataFiles/ad_matrix.mat', 'ad_matrix');

    %%% Transform initial and final position into new nodes

    % Load reference path data
    load('DataFiles/nodes.mat', 'nodes');
    ...
    step_p = 10;
    checkpoints = final_path(step_p:step_p:end,:);
    ...
    %%% Save trajectory (pixels)

    % Finally, discretize the path into 2000 points and save it
    path_p_ref = zeros(n_points,3);

    for i = 1:n_points
        path_p_ref(i,:) = [y(i,1), y(i,2), theta(i,1)];
    end

    save('DataFiles/path_p_ref.mat', 'path_p_ref');

    %%& Save trajectory (meters)

    % We also save a version in meters so that it can be used by the control
    % group
    path_m_ref = zeros(n_points,3);
